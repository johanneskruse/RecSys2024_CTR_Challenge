...
dataset_version = f"Ebnerd_{split}_x1"
image_emb_path = root_path.joinpath(
    "artifacts", "Ekstra_Bladet_image_embeddings", "image_embeddings.parquet"
)
contrast_emb_path = root_path.joinpath(
    "artifacts", "Ekstra_Bladet_contrastive_vector", "contrastive_vector.parquet"
)
MAX_SEQ_LEN = 50

print("Preprocess news info...")
train_news_file = train_path.parent.joinpath("articles.parquet")
train_news = pl.scan_parquet(train_news_file)
test_news_file = test_path.parent.joinpath("articles.parquet")
...
def map_feat_id_func(df, column, seq_feat=False):
    ...
    if seq_feat:
        df = df.with_columns(
            pl.col(column).apply(lambda x: [map_dict.get(i, 0) for i in x])
        )
    else:
        df = df.with_columns(
            pl.col(column).apply(lambda x: map_dict.get(x, 0)).cast(str)
        )
    return df
...
def tokenize_seq(df, column, map_feat_id=True, max_seq_length=5, sep="^"):
    ...
    df = df.with_columns(
        pl.col(column).apply(lambda x: f"{sep}".join(str(i) for i in x))
    )
    return df
...
news = news.with_columns(
    subcat1=pl.col("subcategory").apply(lambda x: str(x[0]) if len(x) > 0 else "")
).collect()
...
def join_data(data_path):
    ...
    history_df = history_df.rename(
        {
            "article_id_fixed": "hist_id",
            "read_time_fixed": "hist_read_time",
            "impression_time_fixed": "hist_time",
            "scroll_percentage_fixed": "hist_scroll_percent",
        }
    )
    history_df = tokenize_seq(
        history_df, "hist_id", map_feat_id=False, max_seq_length=MAX_SEQ_LEN
    )
    ...
    history_df = history_df.with_columns(
        pl.col("hist_id")
        .apply(lambda x: "^".join([news2cat.get(i, "") for i in x.split("^")]))
        .alias("hist_cat"),
        pl.col("hist_id")
        .apply(lambda x: "^".join([news2subcat.get(i, "") for i in x.split("^")]))
        .alias("hist_subcat1"),
        pl.col("hist_id")
        .apply(lambda x: "^".join([news2sentiment.get(i, "") for i in x.split("^")]))
        .alias("hist_sentiment"),
        pl.col("hist_id")
        .apply(lambda x: "^".join([news2type.get(i, "") for i in x.split("^")]))
        .alias("hist_type"),
    )
    ...
    if "test" in data_path:
        sample_df = sample_df.rename({"article_ids_inview": "article_id"}).explode(
            "article_id"
        )
        sample_df = sample_df.with_columns(
            pl.lit(None).alias("trigger_id"), pl.lit(0).alias("click")
        )
    else:
        sample_df = (
    ...
    sample_df = (
        ...
        .with_columns(
            publish_days=(pl.col("impression_time") - pl.col("published_time"))
            .dt.days()
            .cast(pl.Int32),
            publish_hours=(pl.col("impression_time") - pl.col("published_time"))
            .dt.hours()
            .cast(pl.Int32),
...
item_dict = {"key": image_emb_df["article_id"].cast(str), "value": image_emb}
...
contrast_emb = pca.fit_transform(
    np.array(contrast_emb_df["contrastive_vector"].to_list())
)
print("contrast_emb.shape", contrast_emb.shape)
item_dict = {"key": contrast_emb_df["article_id"].cast(str), "value": contrast_emb}
...
# test_df = join_data(data_path=test_path.as_posix())
# print(test_df.head())
